...
from NetworkArchitectures import NetworkAccelerometer
...
for i in range(x_test.shape[0]):
    for c in range(6):
        x_test[i,:,c] -= scipy.signal.wiener(x_test[i,:,c], 30)
        
for i in range(x_train.shape[0]):
    for c in range(6):
        x_train[i,:,c] -= scipy.signal.wiener(x_train[i,:,c], 30)

# The accelerometer is stored in vectors of 1000 elements. Keep only 256
New_bufsiz = x_train.shape[1]
...
model = NetworkAccelerometer(New_bufsiz, Nchannels)
model.summary()
